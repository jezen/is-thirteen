...
tap.test('ocr 13', function(tap) {
    is("http://4.bp.blogspot.com/-Fkj6QOEml9Y/UtPoI55yADI/AAAAAAAAArI/5SNIh5iGxlc/s1600/number-13%5B1%5D.png", function(err, obj) { 
        tap.equal(obj.thirteen() , true); 
        tap.end();
    });
});

tap.test('ocr not 13', function(tap) {
       is("http://www.oar2.org/events/assets/12.png", function(err, obj) {
        tap.equal(obj.thirteen() , false); /// 12 is not 13!!!
        tap.end();
    })
});
